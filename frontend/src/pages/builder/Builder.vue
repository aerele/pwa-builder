<template>
  <div class="h-screen w-screen main flex flex-col">
    <div class="sticky top-0 bg-white shadow-md z-10 flex justify-between p-4">
      <h1>PWA Demo</h1>
      <div>
    ...
    <div class="flex flex-row flex-grow overflow-hidden">
      <div
        class="border-2 rounded-lg border-gray-400 h-full w-[20%] m-4 drop-shadow-lg overflow-y-auto"
      >
        <FormList
          :doctypeList="doctypeList"
          :pwaForm="pwaForm"
          @clicked="handleFormFields"
          :projectName="props.id"
        />
      </div>
      <div
        class="w-[60%] border-2 rounded-lg border-gray-400 mx-4 mt-4 drop-shadow-lg overflow-y-auto"
      >
        <div
          v-if="formData.doctype_name"
          class="flex justify-between items-center shadow-sm sticky top-0 bg-white border-b px-3 py-2 z-10"
        >
          <h2 class="text-2xl">{{ formData.doctype_name }}</h2>
          <Button variant="solid" theme="gray" size="md" @click="setFieldList"
            >Save</Button
          >
        </div>
        <BuilderCanvas
          class="h-44"
          :formName="formData.doctype_name"
          :fieldList="fieldList"
          @handleSave="setFieldList"
        />
        <div
          v-if="!fieldList.length"
          class="border-2 relative h-44 drag flex justify-center items-center"
        >
          Drag Fields Here
        </div>
      </div>
