<template>
  ...
  <div class="px-4 py-2">
    <!-- {{ item }} -->
    <div class="flex flex-row p-4 border-b-0 justify-between items-center cursor-pointer">
      ...
      <MenuItems class="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
        <div class="py-1">
          ...
          <form method="POST" action="#">
            <MenuItem v-slot="{ active }">
              <button type="submit" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block w-full px-4 py-2 text-left text-sm']">Delete</button>
            </MenuItem>
          </form>
        </div>
      </MenuItems>
